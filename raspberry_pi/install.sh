...
echo "  Vendite trovate nel database: ${VENDITE_COUNT}"
if [ "${VENDITE_COUNT}" = "0" ] || [ -z "${VENDITE_COUNT}" ]; then
    echo "  Database vuoto, importo struttura iniziale..."
    [ -f "${WEB_DIR}/database.sql" ] && mysql -u"${DB_USER}" -p"${DB_PASS}" "${DB_NAME}" < ${WEB_DIR}/database.sql 2>/dev/null || true
else
    ...
    mysql -u"${DB_USER}" -p"${DB_PASS}" "${DB_NAME}" -e "CREATE TABLE IF NOT EXISTS configurazione (chiave VARCHAR(100) PRIMARY KEY, valore TEXT, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP) ENGINE=InnoDB;" 2>/dev/null || true
fi
